CREATE OR REPLACE PROCEDURE `sap-adapter.scl_sap_control.generate_l1_layer_view_ddl`(IN project_name STRING)
BEGIN
--DECLARE project_name STRING DEFAULT 'dev-cs-1';
...
(select distinct project_name as table_catalog,table_schema,table_name
          from sap-adapter.scl_sap_control.scl_l1_layer_view_columns);
...
(select '%s' as table_catalog,sgv.table_schema,sgv.table_name,sgv.column_name,sgv.ordinal_position,sgv.view_column_name
  from %s.scl_sap_control.scl_l1_layer_view_columns sgv
