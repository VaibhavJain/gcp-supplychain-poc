MERGE INTO `{{ projectid }}.{{ slt_l1_dimension }}.customer_sales_org_dimension` tgt
USING (
with tmp_scr AS(
SELECT  CONCAT(cm.client,'-',cm.customer,'-',cs.sales_org,'-',cs.distr_channel,'-',cs.division) as customer_sales_org_key
       ,cm.client
       ,cm.customer
       ,cm.country
       ,cm.name
       ...
       ,cm.planning_area
       ,cm.settlqtyequirel
       ,cs.customer as customer_knvv
       ,cs.sales_org
       ,cs.distr_channel
       ,cs.division
       ,cs.created_by as created_by_knvv
       ,cs.created_on as created_on_knvv
       ,cs.authorizgroup
       ,cs.del_id_slsarea
       ,cs.cust_stats_grp
       ,cs.ord_blk_sls_ar
       ...
       ,cs.paym_guar_proc
       ,cs.cred_contr_area
       ,cs.sales_block as sales_block_knvv
       ,cs.rounding_off
       ,cs.settlem_mgmt
       ,cs.uom_group
       ,cs.overdeliv_tol
       ...
       ,cs.pod_timeframe
       ,cs.document_index
       ,cs.carrier_notification
       ,cs.purposecomplete_flag as purposecomplete_flag_knvv
       ,cs.inco_version
       ,cs.inco_location1
       ,cs.inco_location2
       ,cs.data_element_extension_for_con
       ,cs.guid
       ,cs.bill_planproc
       ,cs.empts_depos_fr
       ,cs.emp_update_off
       ...
       ,cs.rel_strategy
       ,cs.cust_sup_id
       ,cs.vas_determin_mode
       ,cs.vas_cust_group
       ,cs.group_strategy
       ,cs.sched_strat
       ,cs.fill_rate
       ,cs.aggregation_level
       ,cs.customer_dc
       ,cs.custid_partner
       ,cs.psst_grouping_rule
       ,cs.exclude_from_psst
       ,smad.address_number
       ,smad.valid_from
       ...
       ,CASE WHEN cm.dw_last_update_date > coalesce(cs.dw_last_update_date,CAST('1900-01-01T00:00:00' AS DATETIME)) and cm.dw_last_update_date > coalesce(smad.dw_last_update_date,CAST('1900-01-01T00:00:00' AS DATETIME))
             THEN cm.dw_last_update_date
             WHEN cs.dw_last_update_date > coalesce(cm.dw_last_update_date,CAST('1900-01-01T00:00:00' AS DATETIME)) and cs.dw_last_update_date > coalesce(smad.dw_last_update_date,CAST('1900-01-01T00:00:00' AS DATETIME))
             THEN cs.dw_last_update_date
 ...
 from `{{ projectid }}.{{ slt_l0_staging }}.customer_master` cm
      inner join `{{ projectid }}.{{ slt_l0_staging }}.customer_sales` cs
   on cm.client = cs.client
  and cm.customer = cs.customer
  and cm.dw_active_indicator = cs.dw_active_indicator
  ...
  and cm.dw_active_indicator = smad.dw_active_indicator
where cm.dw_active_indicator = 'Y'
  and (cm.dw_last_update_date >= coalesce((SELECT CASE WHEN lat.full_delta_indicator = 'F' THEN lat.full_load_datetime
                                ...
                                WHERE lat.table_name = 'customer_sales_org_dimesnion'),CAST('1900-01-01T00:00:00' AS DATETIME))
        OR cs.dw_last_update_date >= coalesce((SELECT CASE WHEN lat.full_delta_indicator = 'F' THEN lat.full_load_datetime
                                             WHEN lat.full_delta_indicator <> 'F' THEN lat.delta_load_datetime
                                             ELSE CAST('1900-01-01T00:00:00' AS DATETIME)
                                        END FROM `{{ projectid }}.{{ slt_l0_staging }}.layer1_audit_table` lat
                                WHERE lat.table_name = 'customer_sales_org_dimesnion'),CAST('1900-01-01T00:00:00' AS DATETIME))
...
SELECT NULL as customer_sales_org_join_key,t.*
  from tmp_cso t
       INNER JOIN `{{ projectid }}.{{ slt_l1_dimension }}.customer_sales_org_dimension` pd
    on pd.customer_sales_org_key = t.customer_sales_org_key
   AND pd.finalmd5key <> t.finalmd5key
 WHERE pd.dw_active_indicator = 'Y') scr
ON tgt.customer_sales_org_key = scr.customer_sales_org_join_key
...
VALUES
(
 TO_HEX(MD5(CONCAT(scr.customer_sales_org_key,scr.finalmd5key)))
...
,scr.pod_timeframe
,scr.document_index
,scr.carrier_notification
,scr.purposecomplete_flag_knvv
,scr.inco_version
,scr.inco_location1
,scr.inco_location2
,scr.data_element_extension_for_con
,scr.guid
,scr.bill_planproc
,scr.empts_depos_fr
,scr.emp_update_off
...
,scr.rel_strategy
,scr.cust_sup_id
,scr.vas_determin_mode
,scr.vas_cust_group
,scr.group_strategy
,scr.sched_strat
,scr.fill_rate
,scr.aggregation_level
,scr.customer_dc
,scr.custid_partner
,scr.psst_grouping_rule
,scr.exclude_from_psst
,scr.address_number
,scr.valid_from
,scr.address_version
,scr.valid_to
...
USING ( SELECT 
         ...
         ,CASE WHEN (select full_delta_indicator from `{{ projectid }}.{{ slt_l0_staging }}.layer1_audit_table` where table_name = 'customer_sales_org_dimension') IS NULL
                    OR (select full_delta_indicator from `{{ projectid }}.{{ slt_l0_staging }}.layer1_audit_table` where table_name = 'customer_sales_org_dimension') = 'F'
               THEN 'F'
               ELSE 'D'
          END AS full_delta_indicator
        ,'customer_master,customer_sales,supplier_master_address_details' as source_table_names
        ,CAST('1900-01-01T00:00:00' AS DATETIME) as full_load_datetime
        ,COALESCE((select max(input_last_update_date) as delta_load_datatime from `{{ projectid }}.{{ slt_l1_dimension }}.customer_sales_org_dimension`),CAST('1900-01-01T00:00:00' AS DATETIME))  as delta_load_datetime
        ,(coalesce((select COUNT(*) AS inserted_record_count from `{{ projectid }}.{{ slt_l1_dimension }}.customer_sales_org_dimension` a 
          where a.dw_active_indicator = 'Y'  and a.dw_last_update_date > COALESCE((SELECT CASE WHEN b.full_delta_indicator = 'F' 
                                        ...
                                        where b.table_name = 'customer_sales_org_dimension'),CAST('1900-01-01T00:00:00' AS DATETIME))),(select count(*) as inserted_record_count from `{{ projectid }}.{{ slt_l1_dimension }}.customer_sales_org_dimension` where dw_active_indicator = 'Y'))) as inserted_record_count
