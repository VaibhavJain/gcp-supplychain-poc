CREATE OR REPLACE PROCEDURE `{$projectid}.{$slt_l0_control}.generate_slt_l1_layer_view_ddl`(IN project_name STRING)
BEGIN
--DECLARE project_name STRING DEFAULT '{$projectid}';
...
(select distinct project_name as table_catalog,table_schema,table_name
          from {$projectid}.{$slt_l0_control}.slt_l1_layer_view_columns);
...
(select '%s' as table_catalog,sgv.table_schema,sgv.table_name,sgv.column_name,sgv.ordinal_position,sgv.view_column_name
  from %s.{$slt_l0_control}.slt_l1_layer_view_columns sgv
